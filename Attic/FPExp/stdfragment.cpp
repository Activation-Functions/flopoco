...
void StdFragment::evalpos(int accuracy, int start, int& overlapping, bool& is_signed)
{
  ...
  // calcul des param�tres input_bits et output_bits de la table
  // (si output_bits <= 0, aucune table ne sera construite)
  input_bits = min(reallength, accuracy - 2 * start + 1);
  output_bits = accuracy - 2 * start; // pr�voit large pour stocker e^x-1-x ~ x^2/2 (x premier morceau)
  exp_part1_start = start - 1;        // pr�voit large pour stocker e^x-1 ~ x
  exp_part2_start = end - 1;          // pr�voit pour e^morceaux_suivants-1
  ...
  // param�tres du produit (n'ont un sens que s'il y a un morceau apr�s !)
...
void StdFragment::write_arch(std::string prefix, ostream& o)
{
  ...
  Signal x("x", accuracy, start);		  // entr�e du composant
  Signal part_1("part_1", accuracy, start, end);  // premier morceau de l'entr�e
  ...
  // exponentielle des morceaux suivants, calcul�e avec un autre composant
  // (si c'est le dernier morceau, les signaux sont d�finis mais n'ont aucun sens !)
  ...
  if (next_part == 0)
    ...
    if (product_obits <= 0)
      /* si on arrive l�, il est probable que le d�coupage ne soit pas optimal
         (la derni�re partie est trop petite) */
...
{
  /* fait l'appel r�cursif sur les morceaux suivants
     (oui, mais en c++, on ne peut pas acc�der aux membres prot�g�s des
      objets construits avec la classe m�re qui ne sont pas aussi construits
      avec la classe d�riv�e. on doit donc passer par la classe m�re - ou
      rendre tous les membres publics) */
  Fragment::write_tbl_declaration(prefix, o);
  int input_size = accuracy - start;

  // table de x -> e ^ x - x - 1 avec la pr�cision maximale
  if (output_bits > 0)
...
double StdFragment::max_error(double input_error)
{
  ...
  /* hypoth�se : on suppose que le dernier morceau est assez
     long pour que l'influence de l'erreur input_error sur
     le terme quadratique soit n�gligeable */

  if (output_bits <= 0)
    // approxime e^x par 1+x -> erreur en x^2/2 (termes suivants du DSE n�glig�s)
    fragment_error = negPowOf2(2 * start - accuracy + 1);
  else {
    // erreur sur la valeur tabul�e de e^x-1-x
    fragment_error = 0.5;
    // �ventuellement, erreur quand l'entr�e est tronqu�e
    if (input_bits < reallength)
      /* x tronqu� � input_bits bits. on pose e = 2 ^ -(start + input_bits)
	 erreur max en sortie : (x + e) ^ 2 - x ^ 2 = 2ex + e ^ 2
         input_bits choisi pour avoir 2ex <= 2 ^ -accuracy d'o� */
  ...
  if (next_part == 0)
    ...
    result = result + fragment_error // addition des erreurs pr�c�dentes
             + 1.0                   // les entr�es du produit sont tronqu�es
	     + max_output * result   // erreurs pr�c�dentes, multipli�es
	     + negPowOf2(next_part->getStart()) * fragment_error
	     + 1.0;                  // r�sultat du produit tronqu�
  }
