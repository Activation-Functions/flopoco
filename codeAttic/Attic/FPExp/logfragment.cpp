...
public:
  ...
  void function(double x, bool negative, mpfr_t& result)
  {
    if (negative) x -= delta;
      ...
      mpfr_log(log_val, exp_val, GMP_RNDN);          // calcul du log avec une "grande" pr�cision (par rapport � accuracy)
      mpfr_sub(result, num_6, log_val, GMP_RNDN);    // arrondi au plus proche
      mpfr_add(result, result, value, GMP_RNDD);     // op�ration exacte
      mpfr_sub(result, result, num_6, GMP_RNDD);     // op�ration exacte
      mpfr_clear(value);
      mpfr_clear(exp_val);
      mpfr_clear(log_val);
      mpfr_clear(num_6);
...
/* Op�rations pr�liminaires sur le morceau
   ======================================= */
...
void LogFragment::evalpos(int accuracy, int start, int& overlapping, bool& is_signed)
{
  ...
  /* TODO: si max_input est petit, le minimum pourrait aussi
     �tre atteint dans les n�gatifs puisqu'on arrondit l'exponentielle par
     valeur inf�rieure et qu'elle est prise en une valeur -newPowOf2(start)
     (et pas -newPowOf2(start) + newPowOf2(end)) au minimum */

  // calcule le nombre de bits en sortie de la table de logarithmes
  // TODO: am�liorer cette approximation (pour l'instant il n'y a
...
void LogFragment::write_arch(std::string prefix, ostream& o)
{
  ...
  // entree pour la table x -> exp(x)_arrondi-1-x (en valeur absolue si sign�)
  ...
  /* sortie de la table x -> x - log(exp(x)_arrondi) (x : 1er morceau de l'entr�e)
     comme exp(x) est arrondi par valeur inf�rieur, le r�sultat est positif */
  ...
  // total � ajouter ou soustraire au produit selon le signe
...
// second fichier : tables utilis�s dans le composant

void LogFragment::write_tbl_declaration(std::string prefix, ostream& o)
{
  Fragment::write_tbl_declaration(prefix, o); // fait l'appel r�cursif sur les morceaux suivants
  int input_size = accuracy - start;

  // table de x -> {e ^ x - 1 - x} arrondi � la pr�cision de l'entr�e
  if (exp_bits > 0) {
