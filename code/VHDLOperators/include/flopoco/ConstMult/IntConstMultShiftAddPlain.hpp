#ifndef INTCONSTMULTSHIFTADDPLAIN_HPP
#define INTCONSTMULTSHIFTADDPLAIN_HPP
#include <vector>
#include <sstream>
#include <gmp.h>
#include <mpfr.h>
...
/**
	@brief Integer constant multiplication using shift and add generating Plain VHDL

	See also ShiftAddOp, ShiftAddDag
	ShiftAddOp defines a shift-and-add operation for IntConstMultShiftAddPlain.

	ShiftAddDag deals with the implementation of an IntConstMultShiftAddPlain as a
...
namespace flopoco{
	...
	class IntConstMultShiftAddPlain : public Operator
	{
	public:
		/** @brief The standard constructor, inputs the number to implement */
		IntConstMultShiftAddPlain(OperatorPtr parentOp, Target* target, int xsize, mpz_class n);
		 ...
		 */
		IntConstMultShiftAddPlain(OperatorPtr parentOp, Target* _target, int _xsize, mpz_class n,
		...
		IntConstMultShiftAddPlain(OperatorPtr parentOp, Target* target, int xsize);
