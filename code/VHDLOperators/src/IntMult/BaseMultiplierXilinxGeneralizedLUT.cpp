...
namespace flopoco
{
    ...
    void  BaseMultiplierXilinxGeneralizedLUT::calc_tile_properties(bool x_signed, bool y_signed){
            ...
            for(unsigned long long x=0; x<(1ULL<<xy_dep_list.first.size()); x++){
                ...
                for(int yi=0; yi<xy_dep_list.second.size(); yi++){
                    for(int xi=0; xi<xy_dep_list.first.size(); xi++){
                        sum += coverage[xy_dep_list.first[xi]][xy_dep_list.second[yi]]*(( ((x&(1ULL<<xi))?1:0)*(1ULL<<xy_dep_list.first[xi]))*(((y&(1ULL<<yi))?1:0)*(1ULL<<xy_dep_list.second[yi])))*((x_signed&&(xy_dep_list.first[xi]==coverage.size()-1))?-1:1)*((y_signed&&(xy_dep_list.second[yi]==coverage[0].size()-1))?-1:1);
                    }
                }
                if(sum < 0){
        ...
        for(int i = 0; i < minterms.size(); i++){
            ...
            print_eq(xy_dep_list.first.size(), xy_dep_list.second.size(), simplified[i], xy_dep_list);cout << endl;
        }
    ...
    void BaseMultiplierXilinxGeneralizedLUT::quine_mccluskey(vector<int> &minterms, int input_size, vector<vector<int>> &simplified){
        ...
        do{
            ...
            for(int i = 0; i < level_terms[step_idx].size()-1; i++){               //select class of inputs set to 1
                for(int j = 0; j < level_terms[step_idx][i].size(); j++){          //select minterm within the current class
                    for(int k = 0; k < level_terms[step_idx][i+1].size(); k++){    //select minterm within the subsequent class
                        ...
                        if(diffs==1 && (get<1>(level_terms[step_idx][i][j]) == get<1>(level_terms[step_idx][i+1][k]))){
                            ...
                            for(n = 0; n < terms_comb[i].size(); n++) if(((get<0>(terms_comb[i][n]) & ~get<1>(terms_comb[i][n])) == (get<0>(level_terms[step_idx][i][j]) & ~diff)) && (get<1>(terms_comb[i][n]) == (diff | get<1>(level_terms[step_idx][i][j])))) {break;}   //do not push duplicate terms to list of merged minterms of current simplification step
                            if(n == terms_comb[i].size()){  //term was not already present
        ...
        for(int s = level_terms.size()-1; 0 <= s; s--){             //select simplification step
            for(int c = level_terms[s].size()-1; 0 <= c ; c--){         //select class of set bits
                for(int t = level_terms[s][c].size()-1; 0 <= t ; t--){  //select term
        ...
        for(int j = 0; j < source_list.size(); j++){                        //select primeimplicant
            for(int k = 0; k < get<2>(source_list[j]).size(); k++){    //select minterm covered by current primeimplicant
        ...
        for(int j = 0; j < second_quine_table.size(); j++){             //select primeimplicant
            bool is_core_implicant = false;
            for(int k = 0; k < second_quine_table[j].size(); k++){      //select minterm
                if(second_quine_table[j][k]){
                    int pe_cov = 0;
                    for(int l = 0; l < second_quine_table.size(); l++){ //check for exclusive coverage of minterms by current primeimplicant
                        if(second_quine_table[l][k]) pe_cov++;
                    }
                    if(pe_cov == 1){
                        core_primeimplicants[j] = 1;
                        for(int l = 0; l < get<2>(source_list[j]).size(); l++){    //select minterm covered by current primeimplicant
        ...
        for(int cur_mt = 0; cur_mt < minterms.size(); cur_mt++){            //select minterm
            //if(covered_minterms[cur_mt]) cout << "col=" << cur_mt << " is already covered" << endl;
            if(covered_minterms[cur_mt]) continue;
            for(int comp_mt = 0; comp_mt < minterms.size(); comp_mt++){        //minterm to compare to
                if(covered_minterms[comp_mt] || cur_mt==comp_mt) continue;
                int cur_set=0, ref_set=0;
                for(int pimp = 0; pimp < second_quine_table.size(); pimp++){            //go through the primeimplicants for both minterm to find which is dominant
                    ...
                    if(!second_quine_table[pimp][cur_mt] && second_quine_table[pimp][comp_mt]){cur_set=0; break;}   //current minterm does not use p.i. which the other m.t uses so it cant domiante the other m.t column
                    if( second_quine_table[pimp][cur_mt] && second_quine_table[pimp][comp_mt]){cur_set++; ref_set++;};
        ...
        for(int cur_pimp = 0; cur_pimp < second_quine_table.size(); cur_pimp++){            //go through the primeimplicants to find which row is dominant
            if(core_primeimplicants[cur_pimp] || skip_primeimplicants[cur_pimp]) continue;
            for(int ref_pimp = 0; ref_pimp < second_quine_table.size(); ref_pimp++){        //go through the primeimplicants to compare the covered minterm to
                if(core_primeimplicants[ref_pimp] || cur_pimp == ref_pimp) continue;
                int cur_set=0, ref_set=0;
                for(int cur_mt = 0; cur_mt < minterms.size(); cur_mt++){                    //select minterm
                    if(covered_minterms[cur_mt]) continue;
                    //cout << "row=" << pimp << " cur=" << second_quine_table[pimp][cur_mt] << " ref=" << second_quine_table[pimp][comp_mt] << " skip=" << (!second_quine_table[pimp][cur_mt] && second_quine_table[pimp][comp_mt]) << endl;
                    if(!second_quine_table[cur_pimp][cur_mt] && second_quine_table[ref_pimp][cur_mt]){cur_set=0; break;}   //current minterm does not use p.i. which the other m.t uses so it cant domiante the other m.t column
                    if( second_quine_table[cur_pimp][cur_mt] && second_quine_table[ref_pimp][cur_mt]){cur_set++; ref_set++;};
        ...
        for(int pimp = 0; pimp < second_quine_table.size(); pimp++){
            ...
            for(int cur_mt = 0; cur_mt < minterms.size(); cur_mt++){                    //select minterm
                if(covered_minterms[cur_mt]) continue;
                if(second_quine_table[pimp][cur_mt]){used_primeimplicants_list.push_back(pimp); break;}
            }
        }
    ...
    void BaseMultiplierXilinxGeneralizedLUT::count_required_LUT(vector<vector<vector<int>>> &eqs, vector<int> n_dependencys){
        ...
        for(int combi = 0; combi < possible_combis.size(); combi++){
            ...
            for(int combi1 = 0; combi1 < possible_combis.size(); combi1++){
                ...
                int placed_combi = 0;
                for(; placed_combi < test_combis.size(); placed_combi++){       //eq may not be already present
    ...
    BaseMultiplierXilinxGeneralizedLUTOp::BaseMultiplierXilinxGeneralizedLUTOp(Operator *parentOp, Target* target, bool isSignedX, bool isSignedY, const vector<vector<int>> &coverage, const pair<vector<int>,vector<int>> &xy_dependency_list, const vector<vector<vector<int>>> &equations, const vector<pair<int,int>> &combined_eqs, const unsigned usedOutBits) : Operator(parentOp,target), wX(coverage.size()), wY(coverage[0].size()), isSignedX(isSignedX), isSignedY(isSignedY), coverage(coverage)
    {
        ostringstream name;
                    ...
                    if(combined_eqs[comb_eq].first == eq || combined_eqs[comb_eq].second == eq){
                        ...
                        for(int t=0; t<equations[combined_eqs[comb_eq].first].size(); t++){
                            ...
                            for(int v=0; v<equations[combined_eqs[comb_eq].first][t].size(); v++){
                                ...
                                if(equations[combined_eqs[comb_eq].first][t][v]<0){ //var is inverted
                                    if(v == 0){
                        ...
                        for(int t=0; t<equations[combined_eqs[comb_eq].second].size(); t++){
                            ...
                            for(int v=0; v<equations[combined_eqs[comb_eq].second][t].size(); v++){
                                ...
                                if(equations[combined_eqs[comb_eq].second][t][v]<0){ //var is inverted
                                    if(v == 0){
                                        term = ~lut_in(lutInpMaps[idx]);
                                        cout << ((t != 0)?"||":"") << "!i" << lutInpMaps[idx];
                                    } else{
                        ...
                        for(;cvars<common_vars.size();cvars++){
                            inPortMap(join("i", cvars), ((common_vars[cvars]-1 < xy_dependency_list.first.size())?join("X", of(xy_dependency_list.first[common_vars[cvars]-1])):join("Y", of(xy_dependency_list.second[common_vars[cvars]-xy_dependency_list.first.size()-1]))) );
                        }
                        for(;cvars<6;cvars++){
                ...
                if((realized_eqs & (1<<eq)) == 0){
                    ...
                    for(int t=0; t<equations[eq].size(); t++){
                        ...
                        for(int v=0; v<equations[eq][t].size(); v++){
                            ...
                            if(equations[eq][t][v]<0){ //var is inverted
                                if(v == 0){
                                    term = ~lut_in(lutInpMaps[idx]);
                                    cout << ((t != 0)?"||":"") << "!i" << lutInpMaps[idx];
                                } else{
...
/*        lut_op test, test1;
    ...
    TestList BaseMultiplierXilinxGeneralizedLUTOp::unitTest(int index)
    {
