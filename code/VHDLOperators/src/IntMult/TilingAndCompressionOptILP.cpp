...
void TilingAndCompressionOptILP::solve()
{
    ...
    void TilingAndCompressionOptILP::compressionAlgorithm() {
		...
		vector<vector<int>> zeroInputsVector(s_max, vector<int>((int)prodWidth+4 ,0));    //The GPCs can protrude 4 bit over the MSB of the BH
		vector<vector<vector<int>>> row_adder(s_max, vector<vector<int>>((int)prodWidth+4, vector<int>(3*3, 0)));   //3 types of row adders that have 3 (L,M,R) elements (3x3=9)
...
//                            if((wOut < (int)prodWidth) && ((xs+tiles[s]->wX()+ys+tiles[s]->wY()-2) < ((int)prodWidth-wOut-guardBits))) break;
    ...
    if(performOptimalTruncation && (wOut < (int)prodWidth))
    {
        ...
        ScaLP::Constraint cLimConstraint = Cvar <= errorBudget - 1;
    ...
    for(int s = 0; s <= s_max; s++){
        ...
        vector<vector<ScaLP::Term>> rcdDependencies(prodWidth + 1, vector<ScaLP::Term>(3));                       //One term for every column and type of row-adder

        for(int c = 0; c <= prodWidth; c++){        //one bit more for carry of ripple carry adder
...
//                    if(s != s_max-1 && (possibleCompressors[e]->type == CompressorType::Variable)) continue;
                    stringstream nvarName;
...
//                    if(possibleCompressors[e]->type == CompressorType::Variable) cout << "cost=" << possibleCompressors[e]->area << " name=" << nvarName.str() << endl;
            ...
            if(s == 0 && c <= (int)prodWidth)
                C0_bithesp_input_bits(s, c, bitsinCurrentColumn, bitsinColumn, bitsInColAndStage, cvBits);
...
void TilingAndCompressionOptILP::C0_bithesp_input_bits(int s, int c, vector<ScaLP::Term> &bitsinCurrentColumn, vector<ScaLP::Term> &inpBitsinColumn, vector<vector<ScaLP::Variable>> &bitsInColAndStage, vector<ScaLP::Variable> &cvBits){
        ScaLP::Term bitsinColumn;

        if(s==0) bitsinColumn = inpBitsinColumn[c];
        bitsinColumn.add(bitsInColAndStage[s][c], -1);      //Add variable to account for the output bits from sub-multipliers
        bitsinCurrentColumn[c].add(bitsInColAndStage[s][c], -1);
        if(s==0 && cvBits[c] != nullptr && c != prodWidth) bitsinColumn.add(cvBits[c], 1);      //Output bits from constant vector
    ...
    void TilingAndCompressionOptILP::C1_compressor_input_bits(int s, int c, vector<ScaLP::Term> &bitsinCurrentColumn, vector<vector<ScaLP::Variable>> &bitsInColAndStage){
    ...
    void TilingAndCompressionOptILP::C3_limit_BH_height(int s, int c, vector<vector<ScaLP::Variable>> &bitsInColAndStage, bool consider_final_adder){
    ...
    void TilingAndCompressionOptILP::handleRowAdderDependencies(const ScaLP::Variable &tempV, vector<vector<ScaLP::Term>> &rcdDependencies, unsigned int c, unsigned int e) const {
        if(possibleCompressors[e]->type == CompressorType::Variable){
            ...
            if(c < (unsigned)prodWidth + 4 - 1 && (possibleCompressors[e]->subtype == subType::M || possibleCompressors[e]->subtype == subType::R))    //Middle and right element of RCA are counted positive in eq. for relations between RCA elements (C5)
                rcdDependencies[c][possibleCompressors[e]->rcType].add(tempV, 1);
            if(c == (unsigned)prodWidth + 4 && possibleCompressors[e]->subtype == subType::L)                                       //only the left (and not the middle) element of the RCA should be put in the MSB column of the bitheap
                rcdDependencies[c][possibleCompressors[e]->rcType].add(tempV, 1);
        }
    }
    ...
    void TilingAndCompressionOptILP::addRowAdder(){
        {
    ...
    void TilingAndCompressionOptILP::drawBitHeap(){
        vector<vector<int>> bitsOnBitHeap(s_max+1, vector<int>((int)prodWidth+4, 0));
        vector<int> colWidth((int)prodWidth+4, 1);
        ScaLP::Result res = solver->getResult();
