...
namespace flopoco
{
  FixIIRShiftAdd::FixIIRShiftAdd(OperatorPtr parentOp, Target *target, int lsbIn_, int lsbOut_, int msbIn_, int guardBits_, vector<string> coeffb_, vector<string> coeffa_, int64_t shifta_, int64_t shiftb_, string method_, string grapha_,
                                 string graphb_, double H_, double Heps_) :
    Operator(parentOp, target), lsbIn(lsbIn_), lsbOut(lsbOut_), msbIn(msbIn_), guardBits(guardBits_), coeffb(coeffb_), coeffa(coeffa_), shifta(shifta_), shiftb(shiftb_), method(method_), grapha(grapha_), graphb(graphb_), H(H_), Heps(Heps_)
  {
    ...
    if (!isFIR) // wcpg and guard bits only necessary for IIR
    {
      ...
      if (H == 0) // H not set by argument
      {
        ...
        if (!WCPG_tf(&H, coeffb_d_wcpg, coeffa_d_wcpg, n, m, (int) 0))
        THROWERROR("Could not compute H");
      ...
      if (guardBits < 0)
      {
        if (Heps == 0) // if guard bits not set by argument then overwrite
        {
          ...
          THROWERROR("Could not compute Heps");
    ...
    for (uint32_t i = 0; i < n; i++)
    {
      ...
      else if (method == "plain" && isPowerOfTwo(coeffb_si[i], true))
  ...
  TestList FixIIRShiftAdd::unitTest(int testLevel)
  {
