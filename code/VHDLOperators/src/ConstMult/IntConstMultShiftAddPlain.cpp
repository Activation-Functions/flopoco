...
namespace flopoco{
	...
	int compute_total_cost_rec(ShiftAddOp* sao) {
		...
		throw string("This exception in IntConstMultShiftAddPlain::compute_total_cost_rec should never happen");
	}
	 ...
	 */
	void IntConstMultShiftAddPlain::build_pipeline(ShiftAddOp* sao) {
			...
			switch(op) {
				...
				case RSub:
					...
					if(shift==0) { // Add with no shift -- this shouldn't happen with current DAGs so the following code is mostly untested
						if(op==Sub || op==RSub)
							throw string("In IntConstMultShiftAddPlain::build_pipeline, Sub and RSub with zero shift currently unimplemented"); // TODO
	...
	int IntConstMultShiftAddPlain::getArea() {
	...
	IntConstMultShiftAddPlain::IntConstMultShiftAddPlain(OperatorPtr parentOp, Target* _target, int _xsize, mpz_class n) :
		Operator(parentOp, _target), n(n), xsize(_xsize)
	{
  		...
  		if(n == 1) THROWERROR("sorry, multiplication by one is not supported!");
			...
			name <<"IntConstMultShiftAddPlain_"<<xsize<<"_"<<mpz2string(n);
...
#define HACK4ARITH2019 1
	...
	IntConstMultShiftAddPlain::IntConstMultShiftAddPlain(OperatorPtr parentOp, Target* _target, int _xsize, mpz_class n, mpz_class period, int periodMSBZeroes, int periodSize, mpz_class header, int headerSize, int i, int j) :
		Operator(parentOp, _target), xsize(_xsize){
			ostringstream name; 

			srcFileName="IntConstMultShiftAddPlain (periodic)";
	...
	IntConstMultShiftAddPlain::IntConstMultShiftAddPlain(OperatorPtr parentOp, Target* _target, int _xsize) :
		Operator(parentOp, _target), xsize(_xsize)
	{
	...
	IntConstMultShiftAddPlain::~IntConstMultShiftAddPlain() {
		delete implementation;
	}
	...
	string IntConstMultShiftAddPlain::printBoothCode(int* BoothCode, int size) {
		ostringstream s;
	...
	bool IntConstMultShiftAddPlain::findBestDivider(mpz_class n, mpz_t & divider, mpz_t & quotient, mpz_t & remainder) {
	...
	int IntConstMultShiftAddPlain::recodeBooth(mpz_class n, int* BoothCode) {
		int i;
		int *b, *c;
	...
	void  IntConstMultShiftAddPlain::buildMultBooth(){
		int k,i;
		ShiftAddOp *z;
		i=0;
	...
	ShiftAddDag* IntConstMultShiftAddPlain::buildEuclideanTree(const mpz_class n){
	...
	ShiftAddOp* IntConstMultShiftAddPlain::buildEuclideanDag(const mpz_class n, ShiftAddDag* constant){
		...
		if (too_small){
			...
			throw string("ERROR in IntConstMultShiftAddPlain::buildEuclideanDag: Tried to divide a constant lower than 3: this will make no sense. Aborting DAG construction\n");
			return NULL;
		};
	...
	int  IntConstMultShiftAddPlain::prepareBoothTree(mpz_class &n, ShiftAddDag* &tree_try, ShiftAddOp** &level, ShiftAddOp* &result,
	...
	ShiftAddDag*  IntConstMultShiftAddPlain::buildMultBoothTreeFromRight(mpz_class n){
		ShiftAddDag* tree_try= new ShiftAddDag(this);
	 ...
	 */
	// Assumes: implementation is initialized
	ShiftAddDag*  IntConstMultShiftAddPlain::buildMultBoothTreeFromLeft(mpz_class n){
		ShiftAddDag* tree_try= new ShiftAddDag(this);
		int k,j,nk,nonZeroInBoothCode,globalshift; 
		ShiftAddOp *result;
		ShiftAddOp *MX=0;
	 ...
	 */
	// Assumes: implementation is initialized
	ShiftAddDag*  IntConstMultShiftAddPlain::buildMultBoothTreeToMiddle(mpz_class n){
		ShiftAddDag* tree_try= new ShiftAddDag(this);
		int k,j,nk,nonZeroInBoothCode,globalshift; 
		ShiftAddOp *result;
		ShiftAddOp *MX=0;
	 ...
	 */
	// Assumes: implementation is initialized
	ShiftAddDag* IntConstMultShiftAddPlain::buildMultBoothTreeSmallestShifts(mpz_class n){
		ShiftAddDag* tree_try= new ShiftAddDag(this);
	...
	void IntConstMultShiftAddPlain::showShiftAddDag(){
	...
	void IntConstMultShiftAddPlain::showRPAG(){
		string rpag="{";
		for (uint32_t i=0; i<implementation->saolist.size(); i++) {
			if (i>0)
					rpag+=",";
	 ...
	 */
	// Assumes: implementation is initialized
	ShiftAddDag* IntConstMultShiftAddPlain::buildMultBoothTreeBitheap(mpz_class n, int levels)
	{
		ShiftAddDag* tree_try= new ShiftAddDag(this);
	...
	void IntConstMultShiftAddPlain::emulate(TestCase *tc){
	...
	void IntConstMultShiftAddPlain::buildStandardTestCases(TestCaseList* tcl){
		TestCase *tc;
	...
	TestList IntConstMultShiftAddPlain::unitTest(int testLevel)
	{
	...
	OperatorPtr IntConstMultShiftAddPlain::parseArguments(OperatorPtr parentOp, Target *target, vector<string> &args, UserInterface& ui) {
		int wIn;
		string	constant;
		ui.parseStrictlyPositiveInt(args, "wIn", &wIn); 
		ui.parseString(args, "constant", &constant);
		mpz_class constant_mpz(constant); // TODO catch exceptions here?
		return new IntConstMultShiftAddPlain(parentOp, target, wIn, constant_mpz);
	}

	template <>
	const OperatorDescription<IntConstMultShiftAddPlain> op_descriptor<IntConstMultShiftAddPlain> {
	    "IntConstMultShiftAddPlain", // name
