...
namespace flopoco {
  ...
  VaryingPiecewisePolyApprox::VaryingPiecewisePolyApprox(string sollyaString_, double targetAccuracy_, int lsbIn_, int lsbOut_):
	...
	void VaryingPiecewisePolyApprox::createPolynomialsReport()
	{
		...
		REPORT(LogLevel::DETAIL,"  Degree=" << degree	<< "  TableSize=" << sizeInBits(nbInterval)
	...
	template <>
	const OperatorDescription<VaryingPiecewisePolyApprox> op_descriptor<VaryingPiecewisePolyApprox> {
