...
#define DEBUGVHDL 0
	...
	Fix2FP::Fix2FP(OperatorPtr parentOp, Target* target, bool Signed, int MSBI, int LSBI, int wER, int wFR) :
	...
	void Fix2FP::buildStandardTestCases(TestCaseList* tcl){
		...
		OperatorPtr Fix2FP::parseArguments(OperatorPtr parentOp, Target *target, vector<string> &args, UserInterface& ui) {
		int wE, wF, MSB, LSB;
		bool signedO;
	...
	template <>
	const OperatorDescription<Fix2FP> op_descriptor<Fix2FP> {
	    "Fix2FP", // name
