...
namespace flopoco
{
      ...
      : sollyaString(sollyaString_), lsbIn(lsbIn_), lsbOut(lsbOut_), signedIn(signedIn_)
  {
    ...
    if(lsbIn > 0) {
      REPORT(ERROR,
        "ERROR in FixFunctions : You are asking for a function with lsbIn=" << lsbIn << " but the input interval is "
  ...
  void FixFunction::initialize()
  {
    ...
    if((wIn > 0) && (wIn < 17)) {
      ...
      if(wIn > 0) {
        REPORT(DETAIL, "Warning: the minimal output width is evaluated by Sollya and may be overestimated.");
      }
