...
namespace flopoco
{
  ...
  double max(int count, ...)
  {
    ...
    for(int i = 1; i < count; i++) {
      ...
      if(currentVal > currentMax) currentMax = currentVal;  //Requires the type to cast to. Increments ap to the next argument.
    }
    va_end(ap);
  ...
  double min(int count, ...)
  {
    ...
    for(int i = 1; i < count; i++) {
      double currentVal = va_arg(ap, double);
      if(currentVal < currentMin) currentMin = currentVal;  //Requires the type to cast to. Increments ap to the next argument.
    }
    va_end(ap);
  ...
  string range(int left, int right)
  {
    ostringstream o;
    if(left >= right)
      o << "(" << left << " downto " << (right > 0 ? right : 0) << ")";
    else
      o << "(" << (left > 0 ? left : 0) << " to " << right << ")";
  ...
  std::string op_shift(std::string dir, std::string sigin, std::string shift_fac, bool is_signed, std::string resize_fac)
  {
    std::stringstream ss;
    ...
    ss << "std_logic_vector(shift_" << dir << "(" << tmp << ", " << shift_fac << "))";
