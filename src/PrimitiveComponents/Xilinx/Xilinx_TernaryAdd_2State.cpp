...
namespace flopoco
{
    ...
    : Operator(parentOp, target), wIn_(wIn), bitmask_(bitmask), bitmask2_(bitmask2)
  {
    ...
    declare("z", wIn);
    declare( "bbus", wIn+1 );
    vhdl << tab << declare("bbus_in", wIn) << " <= bbus" << range(wIn - 1, 0) << ";" << endl;
    vhdl << tab << "bbus" << range(wIn, 1) << " <= " << declare("bbus_out", wIn) << ";" << endl;
    declare("carry_cct");
    ...
    if (num_slices == 1)
    {
      declare("sum_out_1", wIn);
      newInstance("Xilinx_TernaryAdd_2State_slice",
                  "slice_i",
                  "wIn=" + to_string(wIn) + " is_initial=true lut_content=" + lut_content,
                  "x_in => x,y_in => y,z_in => z,sel_in => sel_i,bbus_in => bbus_in,carry_in => carry_cct",
                  "bbus_out => bbus_out,carry_out => carry" + of(0) + ",sum_out => sum_out_1",
                  "");

      vhdl << tab << "sum_o" + range(wIn - 1, 0) << " <= sum_out_1;" << endl;
