...
\begin{flopocobox}{Operation-specific testBench generation}
  ...
  In addition, the random number generator used there is specific to the operator under test: with two random bit strings as inputs, the probability of a cancellation in the addition (see \sect{sec:fpadd} \page{sec:fpadd}) would be quite low, since it only happens when the exponents differ at most by 1.
